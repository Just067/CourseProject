...
public class MainWindowViewModel
{
    ...
    public MainWindowViewModel(MainWindow hostWindow, ServiceStationController controller) {
        ...
        hostWindow.TblMessage.Text = "Запросы выполнены";
    } // MainWindowViewModel

    public void FillDataGrids()
    {
        HostWindow.DgClients.ItemsSource = _controller.GetAllClients();
        HostWindow.DgAgents.ItemsSource = _controller.GetAllAgents();
    ...
    public RelayCommand ExitCommand => new(
        _ => Application.Current.Shutdown(0),
        _ => true
    );
    ...
    private void Query01Exec() {
        ...
        var clients = _controller.Query01(passport);

        new ResultClientsWindow(clients).ShowDialog();
    } // Query01Exec

    // команда выполнения запроса 1
    public RelayCommand Query01Command => new(o => Query01Exec(), o => true);
    ...
    private void Query04Exec()
    {
        ...
        new ResultAgentsWindow(agents).ShowDialog();
    } // Query04Exec
